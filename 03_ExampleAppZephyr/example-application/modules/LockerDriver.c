...
uint8_t LockerDriver_OpenLocker(uint8_t LockerNum) {
    ...
    lockerControlled = LockerNum;
    isControlOngoing = true;
    LOG_INF("Opened locker %d", LockerNum);
...
void LockerDriver_Loop(void) {
    if (isInitializedLD) {
        if (isControlOngoing) {
            if ((UtilDriver_GetTick() - controlStartTick > MIN_CONTROL_TIME) &&
                ((UtilDriver_GetTick() - controlStartTick > MAX_CONTROL_TIME) ||
                 (LockerStatusReader_GetLockerStatus(lockerControlled) == 1))) {
                resetMuxControl();
                isControlOngoing = false;
                controlStartTick = 0;
                LOG_INF("Closing locker");
            }
        }
    }
}
...
K_THREAD_DEFINE(lckrdriver_id, STACKSIZE, mainThreadLockerDriver, NULL, NULL,
                NULL, PRIORITY, 0, 0);
